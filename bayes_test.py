...
class BayesianHypothesisTestSkewNormal:
    ...
    def run_model(self, draws=2000, tune=1000):
        ...
        with model:
            ...
            # Prior assumption of the standard deviation
            group1_sd = pm.Normal('Group 1 SD', mu=sigma_both, sigma=1)
            group2_sd = pm.Normal('Group 2 SD', mu=sigma_both, sigma=1)
            ...
            self.samples = pm.sample_posterior_predictive(self.trace)
        
        self.sample_posterior_distribution()
    
    def sample_posterior_distribution(self):
        group1_data = np.array(self.samples.posterior_predictive['Group 1 data']).flatten()
        group2_data = np.array(self.samples.posterior_predictive['Group 2 data']).flatten()
